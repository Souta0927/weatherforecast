...
def get_weather_data(latitude, longitude, location_name):
    ...
    custom_df.loc[is_occasional_rain, '天気予報'] = '☁️曇り時々雨' # 👈 修正: 絵文字追加
    
    # 3. 【NEW】晴れ時々曇り (降水 0.0mm かつ 50% <= 日照率 <= 90%)
    is_partly_cloudy_final = is_no_precip & is_partly_cloudy
    # 雨注意や時々雨に該当しない行にのみ適用
    custom_df.loc[is_partly_cloudy_final & (~is_rain_warning) & (~is_occasional_rain), '天気予報'] = '🌤️晴れ時々曇り' # 👈 修正: 絵文字追加
    
    # 4. 曇り (降水 0.0mm かつ 日照率50%未満)
    is_cloudy = is_no_precip & is_low_sunshine
    # 上記1, 2, 3の判定が適用されていない行にのみ適用
    custom_df.loc[is_cloudy & (~is_rain_warning) & (~is_occasional_rain) & (~is_partly_cloudy_final), '天気予報'] = '☁️曇り' # 👈 修正: 絵文字追加
